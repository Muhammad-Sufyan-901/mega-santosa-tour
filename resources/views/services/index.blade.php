...
@section('content')
    <section class="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
        ...
        {{-- Sewa Mobil Section --}}
        <div class="flex justify-between items-center mb-6" id="sewa-mobil">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            @if ($sewaMobilServices->count() > 0)
                @foreach ($sewaMobilServices as $service)
        ...
        @if ($sewaMobilServices->hasPages())
            <div class="flex justify-center mb-8">
                {{ $sewaMobilServices->links() }}
            </div>
        @endif

        {{-- Paket Tour Section --}}
        <div class="flex justify-between items-center mb-6" id="paket-tour">
            <h3 class="text-3xl tracking-tight font-extrabold text-gray-900">
                Paket Tour
            </h3>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @if ($paketTourServices->count() > 0)
                @foreach ($paketTourServices as $service)
                    <div class="max-w-sm bg-white border border-gray-200 rounded-4xl shadow-sm">
                        <a href="{{ route('services.detail', $service->id) }}">
                            @if ($service->thumbnail)
                                <img class="rounded-t-4xl w-full h-64 object-cover"
        ...
        @if ($paketTourServices->hasPages())
            <div class="flex justify-center mt-8">
                {{ $paketTourServices->links() }}
            </div>
        @endif
    </section>
