...
@section('content')
    <div
        ...
        <div class="w-full mb-1">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500"
                                    aria-current="page">Layanan</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Data Layanan</h1>
            </div>
            ...
            <!-- Error Message -->
            @if (session('error'))
                <div class="p-4 mb-4 text-sm rounded-lg bg-red-800 text-white absolute top-8 right-4" role="alert">
            ...
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                <div class="flex items-center mb-4 sm:mb-0">
                    <form class="sm:pr-3" action="{{ route('admin.services.index') }}" method="GET" id="search-form">
                        <label for="services-search" class="sr-only">Cari</label>
                        <div class="relative w-48 mt-1 sm:w-64 xl:w-96">
                            <input type="text" name="search" id="services-search" value="{{ request('search') }}"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @forelse($services as $service)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                            <input id="checkbox-{{ $service->id }}" type="checkbox"
                                                class="w-4 h-4 border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300">
                                            <label for="checkbox-{{ $service->id }}" class="sr-only">checkbox</label>
                                        </div>
                                    </td>
                                    <td class="p-4">
                                        <img src="{{ $service->thumbnail ? asset('assets/images/services/' . $service->thumbnail) : 'https://placehold.co/100x100.png' }}"
                                            alt="Thumbnail" class="w-12 h-12 rounded-lg object-cover">
                                    </td>
                                    <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                        <div class="text-base font-semibold text-gray-900 dark:text-white">
                                            {{ $service->title }}</div>
                                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">Layanan</div>
                                    </td>
                                    <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                        <span
                                            class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">
                                            {{ $service->type_of_service ?? 'Tidak Diset' }}
                                        </span>
                                    </td>
                                    <td
                                        class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                                        {{ Str::limit($service->prolog, 60) }}
                                    </td>
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        ...
                                        <a href="{{ route('admin.services.edit', $service->id) }}"
                                            class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300">
                                            <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                                <path
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        Apakah Anda yakin ingin menghapus layanan "<span id="deleteItemName"
