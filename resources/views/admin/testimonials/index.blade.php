...
@section('content')
        ...
        <div class="w-full mb-1">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500"
                                    aria-current="page">Testimoni</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Data Testimoni</h1>
            </div>

            <!-- Success Message -->
            @if (session('success'))
                <div class="p-4 mb-4 text-sm rounded-lg bg-blue-800 text-white absolute top-8 right-4" role="alert">
            ...
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                <div class="flex items-center mb-4 sm:mb-0">
                    <form class="sm:pr-3" action="{{ route('admin.testimonials.index') }}" method="GET" id="search-form">
                        <label for="testimonials-search" class="sr-only">Cari</label>
                        <div class="relative w-48 mt-1 sm:w-64 xl:w-96">
                            <input type="text" name="search" id="testimonials-search" value="{{ request('search') }}"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @forelse($testimonials as $testimonial)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                            <input id="checkbox-{{ $testimonial->id }}" type="checkbox"
                                                class="w-4 h-4 border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300">
                                            <label for="checkbox-{{ $testimonial->id }}" class="sr-only">checkbox</label>
                                        </div>
                                    </td>
                                    <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                        <div class="text-base font-semibold text-gray-900 dark:text-white">
                                            {{ $testimonial->name }}</div>
                                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">Testimonial</div>
                                    </td>
                                    ...
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <div class="flex items-center">
                                            ...
                                            @for ($i = $testimonial->rating; $i < 5; $i++)
                                                <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 mr-1"
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        ...
                                        @if ($testimonial->is_verified == 0)
                                            <button type="button"
                                                onclick="confirmVerify({{ $testimonial->id }}, '{{ $testimonial->name }}')"
                                                class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300">
                                                <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                                        ...
                                        @else
                                            @php
                                                ...
                                                $buttonText =
                                                    $testimonial->status == 'active' ? 'Nonaktifkan' : 'Aktifkan';
                                                $buttonColor =
                                            ...
                                            @endphp
                                            <button type="button"
                                                onclick="confirmToggleStatus({{ $testimonial->id }}, '{{ $testimonial->name }}', '{{ $newStatus }}', '{{ $buttonText }}')"
                                                class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg {{ $buttonColor }} focus:ring-4">
                                                @if ($testimonial->status == 'active')
                                                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd"
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        Apakah Anda yakin ingin menghapus testimoni dari "<span id="deleteItemName"
    ...
    <div id="verifyModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    <svg class="mx-auto mb-4 text-blue-400 w-12 h-12" fill="none" viewBox="0 0 20 20">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" />
                    </svg>
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        Apakah Anda yakin ingin memverifikasi testimoni dari "<span id="verifyItemName"
                            class="font-semibold text-gray-900 dark:text-white"></span>"?
                    </h3>
                    <div class="flex justify-center gap-4">
                        ...
                        <button onclick="closeVerifyModal()" type="button"
                            class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100">
                            Batal
                        </button>
                    </div>
    ...
    <div id="toggleStatusModal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        Apakah Anda yakin ingin <span id="toggleStatusAction" class="font-semibold"></span> testimoni dari
                        "<span id="toggleStatusItemName" class="font-semibold text-gray-900 dark:text-white"></span>"?
                    </h3>
                    <div class="flex justify-center gap-4">
                        <form id="toggleStatusForm" method="POST" action="">
                            ...
                            <button type="submit"
                                ...
                                Ya, <span id="toggleStatusButtonText"></span>
                            </button>
                        </form>
                        <button onclick="closeToggleStatusModal()" type="button"
                            class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100">
                            Batal
                        </button>
                    </div>
    ...
    <div id="modalBackdrop" class="hidden bg-gray-900/50 fixed inset-0 z-40" onclick="closeAllModals()"></div>
@endsection

@push('js')
    <script>
        // Search functionality
        document.getElementById('testimonials-search').addEventListener('input', function() {
        ...
        function confirmVerify(id, name) {
            document.getElementById('verifyItemName').textContent = name;
            document.getElementById('verifyForm').action = `/admin/testimonials/${id}/verify`;
        ...
        function confirmToggleStatus(id, name, newStatus, buttonText) {
            document.getElementById('toggleStatusItemName').textContent = name;
            document.getElementById('toggleStatusAction').textContent = buttonText.toLowerCase();
            document.getElementById('toggleStatusButtonText').textContent = buttonText;
            document.getElementById('toggleStatusForm').action = `/admin/testimonials/${id}/toggle-status`;
